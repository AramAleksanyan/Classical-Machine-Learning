...
for cls in classes:
    ...
    n_of_samples[cls] = len(class_data)


sum_ = 0
for cls in classes:
    sum_ += (n_of_samples[cls] - 1) * covariance_matrices[cls]

pooled_matrix = sum_ / (n_dataset - n_classes)
...
def discriminant(x_):
    ...
    for cls_ in classes:
        ...
        d = np.dot(np.dot(x_.T, np.linalg.inv(pooled_matrix)), mu_i) + np.log(pi_i)
        d -= 0.5 * np.dot(np.dot(mu_i.T, np.linalg.inv(pooled_matrix)), mu_i)
